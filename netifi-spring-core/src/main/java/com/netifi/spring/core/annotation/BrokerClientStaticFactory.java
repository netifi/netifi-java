...
//                }
                ...
                if (BrokerClientFactory.class.isAssignableFrom(resolvedTargetClass)) {
                    ...
                    for (BrokerClientFactorySupport brokerClientFactory :
                        brokerClientFactories) {
                        if (brokerClientFactory.support(clientClass)) {
                            toRegister = createBrokerClientFactory(
                                clientClass,
                                resolvedTargetClass,
                                brokerClientFactory,
    ...
    public static Tags resolveTags(
        ...
        BrokerClient brokerClientAnnotation) {
        //Tags reconciliation
        TagSupplier tagSupplier = NoTagsSupplier.INSTANCE;
        brokerClientAnnotation.tagSupplier();
        if (!brokerClientAnnotation.tagSupplier()
                                   .equals(NoTagsSupplier.class)) {
    ...
    static <T> T createBrokerClient(com.netifi.broker.BrokerClient brokerClient,
        ...
        BrokerSocket brokerSocket = createBrokerRSocket(brokerClient, routeType, group, destination, tags);
        ...
        if (tracer == null && meterRegistry == null) {
            ...
            Constructor ctor = clientClass.getConstructor(
                RSocket.class,
                MeterRegistry.class,
                Tracer.class
            );
    ...
    public static BrokerSocket createBrokerRSocket(
        ...
        switch (routeType) {
            ...
            case GROUP:
                ...
                break;
            case DESTINATION:
                brokerSocket = brokerClient.groupServiceSocket(
                    group,
                    StringUtils.isEmpty(destination)
                        ? tags
                        : Tags.of("com.netifi.destination", destination)
                              .and(tags)
                );
                break;
        }
