...
public class MessagingRouter implements MutableRouter<MessagingRouter> {
  ...
  public IPCFunction<Mono<Void>> routeFireAndForget(String route) {
    return (payload, decoded) ->
        decoded.isComposite()
            ? this.handle(payload.retain(), FrameType.REQUEST_FNF)
            : this.handle(payload.retain(), route, FrameType.REQUEST_FNF);
  }
  ...
  public IPCFunction<Mono<Payload>> routeRequestResponse(String route) {
    return (payload, decoded) ->
        decoded.isComposite()
            ? handleAndReply(payload, FrameType.REQUEST_RESPONSE, Flux.just(payload.retain()))
                .next()
            : handleAndReply(
                    payload, route, FrameType.REQUEST_RESPONSE, Flux.just(payload.retain()))
                .next();
  }
  ...
  public IPCFunction<Flux<Payload>> routeRequestStream(String route) {
    return (payload, decoded) ->
        decoded.isComposite()
            ? handleAndReply(payload, FrameType.REQUEST_STREAM, Flux.just(payload.retain()))
            : handleAndReply(payload, route, FrameType.REQUEST_STREAM, Flux.just(payload.retain()));
  }

  @Override
  public IPCChannelFunction routeRequestChannel(String route) {
    return (source, payload, decoded) ->
        decoded.isComposite()
            ? handleAndReply(payload, FrameType.REQUEST_CHANNEL, source)
            : handleAndReply(payload, route, FrameType.REQUEST_CHANNEL, source);
  }

  @Override
  public MessagingRouter withFireAndForgetRoute(String route, IPCFunction<Mono<Void>> function) {
    return null;
  }

  @Override
  public MessagingRouter withRequestResponseRoute(
      String route, IPCFunction<Mono<Payload>> function) {
    return null;
  }

  @Override
  public MessagingRouter withRequestStreamRoute(String route, IPCFunction<Flux<Payload>> function) {
    return null;
  }

  @Override
  public MessagingRouter withRequestChannelRoute(String route, IPCChannelFunction function) {
    return null;
  }
  ...
  private Mono<Void> handle(Payload payload, String route, FrameType frameType) {
    MessageHeaders headers = createHeaders(route, frameType, null);
    DataBuffer dataBuffer = retainDataAndReleasePayload(payload);
    int refCount = refCount(dataBuffer);
    Message<?> message = MessageBuilder.createMessage(dataBuffer, headers);
    return Mono.defer(() -> this.messageHandler.handleMessage(message))
  ...
  private Flux<Payload> handleAndReply(
      Payload firstPayload, String route, FrameType frameType, Flux<Payload> payloads) {
    MonoProcessor<Flux<Payload>> replyMono = MonoProcessor.create();
    MessageHeaders headers = createHeaders(route, frameType, replyMono);
  ...
  private MessageHeaders createHeaders(
      String route, FrameType frameType, @Nullable MonoProcessor<?> replyMono) {
    ...
    headers.setHeader(
        ...
        this.routeMatcher.parseRoute(route));
