...
public class BrokerClientMessagingAutoConfiguration {
	...
	public MessageHandlerAcceptor messageHandlerAcceptor(
		...
		acceptor.getArgumentResolverConfigurer()
		        .getCustomResolvers()
		        .removeIf(r -> r instanceof RSocketRequesterMethodArgumentResolver);

		acceptor.getArgumentResolverConfigurer()
		        .addCustomResolver(new BrokerClientRequesterMethodArgumentResolver(
			        properties.getName(),
		        	brokerClient,
			        factory,
			        rSocketStrategies
		...
		brokerClient.addNamedRSocket(properties.getName(), acceptor.apply(STUB_RSOCKET));
